...
def predict_proposed(weight, use_haptic, use_audio, use_virbo, filelist):
    ...
    resultlist = [
        [(frame.squeeze().permute([1, 2, 0]).cpu().detach().numpy() * 255).astype(np.uint8) for frame in result]
        for result in resultlist]

    gt = [[(frame.squeeze().permute([1,2,0]).cpu().detach().numpy()*255).astype(np.uint8) for frame in result]
                  for result in gt]
    return resultlist, gt
...
if __name__ == '__main__':
    ...
    resultlist, gtlist = predict_proposed(Experiement_object_based_all_behaviors['weight_use_haptic'], True, False, False, filelist)
    for idx, result in enumerate(resultlist):
